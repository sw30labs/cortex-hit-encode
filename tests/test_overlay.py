...
from cortex_hit_encode.config import OverlayConfig
from cortex_hit_encode.overlay import (
    OverlayError,
    hold_schedule,
    load_run,
    overlay_from_run,
...
from cortex_hit_encode.render import format_pip_label, rounded_rect_alpha
...
def test_monitor_frame_is_black_plate_with_green_bezel():
    from cortex_hit_encode.render import apply_monitor_frame
...
def test_compose_ffmpeg_overlays_a_tiny_clip(tmp_path):
    from cortex_hit_encode.overlay import compose_ffmpeg, _write_blank_png
...
def test_render_pip_card_is_rgba_with_clear_corners(tmp_path):
    from cortex_hit_encode.config import FSAVERAGE5_VERTICES, RenderConfig
    from cortex_hit_encode.render import render_pip_card
...
def test_pose_index_wraps_and_respects_dps():
    from cortex_hit_encode.spin import clamp_dps, n_poses, pose_blend, pose_index
...
def test_vertex_palette_quiet_stays_quieter_than_loud():
    from cortex_hit_encode.spin import vertex_palette
...
def test_composite_smooth_blends_two_poses():
    from cortex_hit_encode.spin import SpinAtlas, composite_frame_smooth
...
def test_composite_frame_lookup_and_alpha():
    from cortex_hit_encode.spin import SpinAtlas, composite_frame
...
def test_iter_spin_coalesces_identical_keys():
    from cortex_hit_encode.config import OverlayConfig
    from cortex_hit_encode.spin import SpinAtlas, iter_spin_frames
...
def test_bake_atlas_ids_and_transparency(tmp_path):
    from cortex_hit_encode.spin import bake_atlas, composite_frame, vertex_palette
