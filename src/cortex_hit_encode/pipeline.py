...
from cortex_hit_encode.config import RenderConfig, RunConfig, write_manifest
from cortex_hit_encode.patches import whisperx_cpu_compat
...
def predict(run: RunConfig) -> tuple[np.ndarray, list[float]]:
    ...
    """
    from cortex_hit_encode.model import load_model
...
def run(run_cfg: RunConfig, render_cfg: RenderConfig) -> RunResult:
    ...
    if run_cfg.save_predictions:
        ...
        # matrix means `che draw` can retry the picture for free.
