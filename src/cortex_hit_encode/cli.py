...
Five verbs (no command deck):

    doctor   what's broken before you waste an hour finding out
    fetch    pull every weight up front (only if they are not already local)
    render   stimulus -> brain-map stills
...
from cortex_hit_encode import __version__
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="che",
        description=(
            "Encode a stimulus with TRIBE v2 and draw predicted average-subject "
            "cortical maps. Self-contained instrument for cortex-hit-encode."
        ),
    )
    p.add_argument("--version", action="version", version=f"che {__version__}")
    ...
    d.add_argument(
        "--renderer",
        action="store_true",
        help="draw path only — skip torch, tribev2, ffmpeg extras, disk, HuggingFace",
    )
    ...
    f.add_argument(
        ...
        default=None,
        help=(
            "HuggingFace hub cache override. Default: honor HUGGINGFACE_HUB_CACHE "
            "or HF_HOME (a private data root — not a VideoCortex tree)."
        ),
    )
    ...
    r.add_argument(
        ...
        default=None,
        help=(
            "TRIBE feature cache. Default: $CORTEX_HIT_ENCODE_CACHE or ./.che-cache"
        ),
    )
...
def cmd_fetch(a: argparse.Namespace) -> int:
    from cortex_hit_encode import weights
    from cortex_hit_encode.config import default_hf_hub_cache
    ...
    hub = a.cache_dir or default_hf_hub_cache()
    print(f"HuggingFace hub cache: {hub}")
    print("  (set HF_HOME / HUGGINGFACE_HUB_CACHE to a private data root)")
    ...
    if failed:
        print(
            "\nSome encoders did not download. Prefer "
            "`scripts/import-weights.sh` from an existing VideoCortex/HF cache "
            "instead of a fresh download. Llama-3.2-3B is gated: accept the\n"
...
def cmd_render(a: argparse.Namespace) -> int:
    from cortex_hit_encode import pipeline
    ...
    run_cfg = RunConfig(
        ...
        cache_dir=a.cache_dir if a.cache_dir is not None else default_feature_cache(),
...
def cmd_overlay(a: argparse.Namespace) -> int:
    from cortex_hit_encode.overlay import OverlayError, overlay_from_run
