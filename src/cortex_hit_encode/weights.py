...
Prefer ``scripts/import-weights.sh`` from an existing VideoCortex / HuggingFace
cache. These helpers honor ``HF_HOME`` / ``HUGGINGFACE_HUB_CACHE`` when
``cache_dir`` is omitted — point those at a *private* data root, not a
VideoCortex tree. ``che fetch`` is the fallback if you truly have no local copy.
...
from cortex_hit_encode.config import FEATURE_EXTRACTORS, HF_CHECKPOINT_REPO
