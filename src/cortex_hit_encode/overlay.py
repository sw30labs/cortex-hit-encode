...
def load_run(run_dir: Path) -> tuple[np.ndarray, np.ndarray, dict]:
    ...
    if not ts_path.is_file():
        raise OverlayError(
            ...
            "Re-run `che render` (newer than the overlay feature)."
        )
    preds = np.load(pred)
...
def render_pip_cards(
    ...
    *,
    ...
    from cortex_hit_encode import render
...
def _overlay_spin(
    ...
    *,
    ...
    from cortex_hit_encode import spin
    from cortex_hit_encode.render import blit_ribbon, compute_limits, format_pip_label
    ...
    ribbon = None
    if cfg.ribbon:
        from cortex_hit_encode.render import draw_energy_ribbon, energy_curve
