...
``che doctor`` finds them in about five seconds.
"""
...
def check_torch() -> list[Check]:
    from cortex_hit_encode.device import probe
    ...
    if info["torch"] is None:
        return [
            Check(
                ...
                "pip install 'cortex-hit-encode[predict]'",
            )
        ]
...
def check_fsaverage5() -> Check:
    ...
    try:
        from cortex_hit_encode.render import load_fsaverage5
...
def check_feature_cache() -> Check:
    path = default_feature_cache()
    raw = os.environ.get(FEATURE_CACHE_ENV, "").strip()
    source = f"${FEATURE_CACHE_ENV}" if raw else f"./{DEFAULT_FEATURE_CACHE_NAME}"
    return Check("feature cache", OK, f"{path} ({source})")


def check_hf_cache() -> Check:
    path = default_hf_hub_cache()
    if "videocortex" in str(path).lower():
        return Check(
            "hf cache", WARN,
            f"{path} looks like a VideoCortex tree — use a private data root",
            "export HF_HOME=/path/to/private/hf  # not a VideoCortex checkout",
        )
    return Check("hf cache", OK, str(path))


def check_local_weights() -> list[Check]:
    """Offline: are the five hub repos already on disk?

    Prefer ``scripts/import-weights.sh`` over a fresh HuggingFace download.
    """
    hub = default_hf_hub_cache()
    repos = [HF_CHECKPOINT_REPO] + [spec["repo"] for spec in FEATURE_EXTRACTORS.values()]
    checks: list[Check] = []
    for repo in repos:
        found = local_hf_repo_dir(repo, hub)
        if found is not None:
            checks.append(Check(f"local:{repo}", OK, str(found)))
        else:
            checks.append(
                Check(
                    f"local:{repo}", FAIL,
                    f"not in {hub}",
                    "scripts/import-weights.sh --from <VideoCortex-or-HF-cache> "
                    "(do not re-download; do not modify VideoCortex)",
                )
            )
    return checks
...
def run_all(*, network: bool = True, model: bool = True) -> list[Check]:
    ...
    if model:
        checks.append(check_feature_cache())
        checks.append(check_hf_cache())
        checks.extend(check_local_weights())
