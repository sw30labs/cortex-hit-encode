...
the picture half; keeping them apart is what lets ``che draw`` re-render
...
FEATURE_CACHE_ENV = "CORTEX_HIT_ENCODE_CACHE"
DEFAULT_FEATURE_CACHE_NAME = ".che-cache"


def default_feature_cache() -> Path:
    """Feature-extractor cache: ``$CORTEX_HIT_ENCODE_CACHE`` or ``./.che-cache``."""
    raw = os.environ.get(FEATURE_CACHE_ENV, "").strip()
    return Path(raw) if raw else Path(DEFAULT_FEATURE_CACHE_NAME)


def default_hf_hub_cache() -> Path:
    """HuggingFace hub cache — private data root, not a VideoCortex tree.

    Honors ``HUGGINGFACE_HUB_CACHE``, else ``$HF_HOME/hub``, else the
    library default ``~/.cache/huggingface/hub``.
    """
...
def hf_hub_dirname(repo: str) -> str:
    """``facebook/tribev2`` → ``models--facebook--tribev2``."""
    return "models--" + repo.replace("/", "--")


def local_hf_repo_dir(repo: str, hub_cache: Path | None = None) -> Path | None:
    """Return the hub snapshot dir if it exists and is non-empty."""
    root = (hub_cache or default_hf_hub_cache()) / hf_hub_dirname(repo)
...
class RunConfig:
    ...
    cache_dir: Path = dataclasses.field(default_factory=default_feature_cache)
...
def write_manifest(
    ...
    *,
    ...
    from cortex_hit_encode import VENDOR_COMMIT, VENDOR_UPSTREAM

    payload: dict[str, tp.Any] = {
        "cortex_hit_encode_version": _version(),
        "instrument": {
            "name": "cortex-hit-encode",
            "cli": "che",
            "vendored_from": VENDOR_UPSTREAM,
            "vendored_commit": VENDOR_COMMIT,
        },
        "run": {
